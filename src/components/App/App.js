...
function App() {
  ...
  // Проверить токен после перезагрузки
  ...
  function handleUpdateUser(data) {
    ...
    .then((updateInfo) => {
      ...
      handleSignSubmitPopup({
        icon: okIcon, 
        tipTitle: "Профиль успешно обновлен"
      });
    })
    .catch((err) => {
        console.log(err);
        handleSignSubmitPopup({
          icon: errorIcon,
          tipTitle: "Во время запроса произошла ошибка. Возможно, проблема с подключением или сервер недоступен. Подождите немного и повторите попытку"
        })
    });
  };
  ...
  // Получить карты на основе ширины
  ...
  const handleMovieSearchClick = async (searchFilter, isShort) => {
    ...
    try {
      ...
      if (searchFilter === undefined || searchFilter === "" || searchFilter === null || searchFilter < 1) {
        ...
        handleSignSubmitPopup({
          icon: errorIcon,
          tipTitle: "Вам нужно ввести ключевое слово"
        })
  ...
  // При получении фильмов мы получаем карты
  useEffect(() => {
    if (allFoundMovies) {
  ...
  // Получение доп количество карт в зависимости от ширины экрана
  ...
  // В конце фильмов убрать кнопку "Еще"
  useEffect(() => {
  ...
  // Добавление/удаление карточек для сохранения в Фильмах
  ...
  // Поиск в разделе Сохраненные карты
  ...
  // При фильтрации проверяем наличие фильма
  useEffect(() => {
    if (savedFoundMovies) {
      setIsLoading(false);
    } else {
  ...
  // При перезапуске получаем статус сохраненного результата поиска
  useEffect(() => {
  ...
  // Выбор списка сохраненных карт на основе фильтра и удаления
  const getMoviesList = () => {

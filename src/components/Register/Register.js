    ...
    return (
        <section className='register'>
            ...
            <form className='register__form-container' onSubmit={handleSubmit}>
                <SignInput text={'Имя'} name={'name'} type={'string'} err={"Должна быть строка от 2х символов, без пробелов и цифр."} isError={isNameError} onChange={useFormData.handleChange} data={useFormData.values?.name}/>

                <SignInput text={'E-mail'} name={'email'} type={'email'} err={"Некорректный e-mail"} isError={isEmailError} onChange={useFormData.handleChange} data={useFormData.values?.email}/>

                <SignInput text={'Пароль'} name={'password'} type={'password'} err={"8+ символов, минимум 1 прописная, строчная, цифра"} isError={isPasswordError} onChange={useFormData.handleChange} data={useFormData.values?.password}/>
